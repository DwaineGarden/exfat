...
static void decompress_upcase(uint16_t* output, const le16_t* source, size_t size)
{
	size_t si, oi;
	...
	for (si = 0, oi = 0; si < size && oi < EXFAT_UPCASE_CHARS; si++)
	{
		...
		if (ch == 0xffff)
		{
			size_t c = (si + 1 < size) ? le16_to_cpu(source[si + 1]) : 1;
			while (c--)
			{
				output[oi] = oi;
				oi++;
	...
	*node = NULL;
	...
	for (;;)
	{
		...
		case EXFAT_ENTRY_UPCASE:
			...
			decompress_upcase(ef->upcase, upcase_comp, upcase_size);
			free(upcase_comp);
			break;
